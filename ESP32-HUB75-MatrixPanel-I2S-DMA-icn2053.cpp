...
MatrixPanel_I2S_DMA* MatrixPanel = NULL;

void MatrixPanel_I2S_DMA::buffersFree()
{
  int i;
...
#ifdef USE_COLORx16
inline void MatrixPanel_I2S_DMA::getRGBColor16(int offset_y, int offset_x, rgb888_t& rgb888)
{
...
IRAM_ATTR void MatrixPanel_I2S_DMA::prepareDmaRows(uint8_t row_offset, uint8_t dma_buff_id)
{
  rgb888_t pixel888_1;
  rgb888_t pixel888_2;
  driver_rgb_t pixel1;
  ...
  for(int row_buf_id = 0; row_buf_id < dma_buff.row_data_cnt; row_buf_id++)
  {
    ...
    { 
      ...
      {
        mask_rgb12 = 0;
        if (pixel1.r < 0) mask_rgb12 |= BIT_R2; pixel1.r <<= 1;
        if (pixel1.g < 0) mask_rgb12 |= BIT_G2; pixel1.g <<= 1;
        if (pixel1.b < 0) mask_rgb12 |= BIT_B2; pixel1.b <<= 1;
        if (pixel2.r < 0) mask_rgb12 |= BIT_R1; pixel2.r <<= 1;
        if (pixel2.g < 0) mask_rgb12 |= BIT_G1; pixel2.g <<= 1;
        if (pixel2.b < 0) mask_rgb12 |= BIT_B1; pixel2.b <<= 1;
...
IRAM_ATTR void MatrixPanel_I2S_DMA::sendCBRow(uint8_t buff_id)
{
...
IRAM_ATTR void MatrixPanel_I2S_DMA::sendCallback()
{
    ...
    {      
      ...
      {         
        ...
        if (rows_send_cnt == rows_per_frame) 
        {
          ...
          prepareDmaRows(row_send_idx, dma_buffer_id);
        ...
        if (rows_send_cnt > dma_buff.row_data_cnt) 
        {
          ...
          prepareDmaRows(row_send_idx - dma_buff.row_data_cnt, dma_buffer_id);
          dma_buffer_id ^= 1;
        }        
      }else
      {
...
void MatrixPanel_I2S_DMA::waitDmaReady()
{
...
void MatrixPanel_I2S_DMA::sendVsync()
{
  waitDmaReady();
...
void MatrixPanel_I2S_DMA::sendFrame(bool waitSend, bool autoVsync)
{
  waitDmaReady();
  //Serial.print(">");
...
bool MatrixPanel_I2S_DMA::allocateDMAmemory()
{
  ...
  */
  ...
  uint8_t buff_cnt; //общее количество DMA буферов
  ...
  { 
    ...
    matrix_buffer_size = (dma_buff.frame_prefix_len*ICN2053_PREFIX_CNT + dma_buff.frame_suffix_cnt*ICN2053_SUFFIX_CNT)*SIZE_DMA_TYPE;
    matrix_buffer_size <<= m_cfg.double_dma_buff;
  ...
  matrix_buffer_size += dma_buff.row_data_len*dma_buff.all_row_data_cnt*SIZE_DMA_TYPE + gamma_table_len;
  ...
  dma_buff.rowBits = (ESP32_I2S_DMA_STORAGE_TYPE**)heap_caps_calloc(buff_cnt,sizeof(void*), MALLOC_CAP_32BIT | MALLOC_CAP_DMA);
  if (dma_buff.rowBits == NULL)
  {
    #ifdef SERIAL_DEBUG
...
void MatrixPanel_I2S_DMA::fillRectFrameBuffer(int16_t x1, int16_t y1, int16_t x2, int16_t y2)
{
  uniptr_t addr;
...
void MatrixPanel_I2S_DMA::fillRectBuffer(int16_t x, int16_t y, int16_t w, int16_t h)
{
  if ( !initialized ) 
  { 
    #ifdef SERIAL_DEBUG 
  ...
  if (x < 0) x = 0;
  if (x2 >= pixels_per_row) x2 = pixels_per_row - 1;
  if (y2 >= m_cfg.mx_height) y2 = m_cfg.mx_height - 1;
  ...
  {
    //shift
    //fillRectDMABuffer(x, y, x2, y2);
  }
}

void MatrixPanel_I2S_DMA::clearDmaBuffer(uint8_t _buff_id)
{
  int y_max, y_min;
...
void MatrixPanel_I2S_DMA::clearBuffer(uint8_t _buff_id)
{  
  _buff_id &= 1;
...
MatrixPanel_I2S_DMA::MatrixPanel_I2S_DMA(bool Virtual):
  #if defined(USE_GFX_ROOT)
  GFX(MATRIX_WIDTH, MATRIX_HEIGHT)
  #elif !defined(NO_GFX)
  Adafruit_GFX((Virtual)?(PANEL_WIDTH*PANEL_COUNT_X):(PANEL_WIDTH*PANEL_COUNT_X*PANEL_COUNT_Y), 
  (Virtual)?(PANEL_HEIGHT*PANEL_COUNT_Y):(PANEL_HEIGHT))
  #endif
{
  memcpy(&m_cfg, &hub75_i2s_cfg_default, sizeof(hub75_i2s_cfg_t));
}

//MatrixPanel_I2S_DMA 
//@param  {HUB75_I2S_CFG} opts : structure with matrix configuration
MatrixPanel_I2S_DMA::MatrixPanel_I2S_DMA(const hub75_i2s_cfg_t& opts,bool Virtual):
  #if defined(USE_GFX_ROOT) 
  GFX(opts.mx_width*opts.chain_length, opts.mx_height),
  #elif !defined(NO_GFX)
  Adafruit_GFX((Virtual)?(opts.mx_width*opts.chain_length_x):(opts.mx_width*opts.chain_length_x*opts.chain_length_y), 
               (Virtual)?(opts.mx_height*opts.chain_length_y):(opts.mx_height)),
  #endif        
  m_cfg(opts)
{    
}

const hub75_i2s_cfg_t& MatrixPanel_I2S_DMA::getCfg() const 
{
  return m_cfg;
};
...
void MatrixPanel_I2S_DMA::stopDMAoutput() 
{  
  clearScreen();
...
bool MatrixPanel_I2S_DMA::begin(hub75_pins_t* hub75_pins_ptr)
{
  // RGB
...
IRAM_ATTR void MatrixPanel_I2S_DMA::flipDMABuffer() 
{         
  if (!m_cfg.double_buff)
  {
...
void MatrixPanel_I2S_DMA::setNegative(bool negative, bool autoSend)
{
  waitDmaReady();
...
void MatrixPanel_I2S_DMA::setDMAGetUserRGB(getUserRGB_p _getUserRGB)
{
  getUserRGB = _getUserRGB;
}

void MatrixPanel_I2S_DMA::setDrawUserPixel(drawUserPixel_p _drawUserPixel)
{
