...
class MatrixPanel_I2S_DMA {
  ...
  public:
    ...
    MatrixPanel_I2S_DMA(bool Virtual = false);
    // MatrixPanel_I2S_DMA 
    // @param  {HUB75_I2S_CFG} opts : structure with matrix configuration    
    // Virtual - используется вычисления рамеров панели для ESP32-VirtualMatrixPanel-I2S-DMA 
    MatrixPanel_I2S_DMA(const hub75_i2s_cfg_t& opts, bool Virtual = false);
    ...
    //!!! для ICN2053 потребуется перерисовка кадра по sendFrame(), или flipDMABuffer()
    void setPanelBrightness(int b);
    void setBrightness8(uint8_t b);

    void setNegative(bool negative, bool autoSend = true);

    //Get a class configuration struct
    const hub75_i2s_cfg_t& getCfg() const;
  ...
  protected:    
    ...
    hub75_i2s_cfg_t m_cfg;// Matrix i2s settings    
